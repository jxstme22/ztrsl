...
SENSEVOICE = {
    ...
    "files": [
        ("model.int8.onnx", 239233841, "c71f0ce00bec95b07744e116345e33d8cbbe08cef896382cf907bf4b51a2cd51", "model"),
        ("tokens.txt", 315894, "f449eb28dc567533d7fa59be34e2abca8784f771850c78a47fb731a31429a1dc", "tokens"),
    ],
}
...
OPUS_EN_ZH = {
    ...
    "files": [
        ("model.bin", 155502615, "f24c2bb82368f7de0196882de1d8b644d2aa54ae2439c3142f263de8a64ea2a9", "model"),
        ("config.json", 215, "ce02c0c0d02f285d2ff34c80b0867ccb5c4a3b250a275e6d1d2884f5499a6e46", "config"),
        ("shared_vocabulary.json", 1303887, "37314a6abb25ed8f8497498aeeb31fcea98de892bf00ff7c2e8c966b26fe0b82", "vocabulary"),
        ("source.spm", 806435, "5775ddc9e3ff2fae91554da56468ad35ff56edaba870fea74447bc7234bfdaa8", "source_spm"),
        ("target.spm", 804600, "81dc94efa84e4025ef38d25d5d07429fe41e3eb29d44003f1db6fe98487b0052", "target_spm"),
    ],
}
...
def download(url: str, dest: Path, expected_sha256: str) -> None:
    if dest.is_file() and hashlib.sha256(dest.read_bytes()).hexdigest() == expected_sha256:
        ...
        return
    print(f"  downloading {dest.name} ({dest.stat().st_size if dest.exists() else 0} bytes on disk)")
